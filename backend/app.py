...
def get_calendar_wise_details(text):
    url = "https://us-central1-aiplatform.googleapis.com/v1/projects/hack-team-smriti-empowerers/locations/us-central1/publishers/google/models/gemini-1.5-pro:generateContent"
    access_token = os.getenv("ACCESS_TOKEN_GCP")
    header = {
        "Authorization" : f"Bearer {access_token}"
    ...
    body={
        "contents": {
            "role": "user",
            "parts": {
                "text": """Consider that you are an assistant and have the task of extracting information like description about an event and the date of the event from an input text string. Do not return any code just the ouput array as mentioned below. So consider that the input will be a sentence for example - "set a reminder for my birthday on 15th may 2024" and the output should be an array like - ["2024-05-15","happy birthday"] where the first element is the date and the second element is the description of the event. The input sentence is - """+ text
            }
        },
        "generation_config": {
            "temperature": 0.9,
    ...
    response = requests.post(url,headers=header,json=body)
    final = response.json()
    finallest = json.loads(final["candidates"][0]["content"]["parts"][0]["text"])
    return (finallest)
...
def get_calender_appointments():
    ...
    reponse = get_calendar_wise_details(transcript)
    return {"data":reponse}
